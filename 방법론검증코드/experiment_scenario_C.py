def main():
    ...
    # --- 1.0. 실험 시나리오 제어 플래그 ---
    # 시나리오 C: 물리적 확장, 회전 변수 제거
    USE_EXPANDED_DATA = True  # 확장된 DB 사용 (시나리오 B의 핵심)
    ENABLE_ROTATION = False   # 기둥 회전 비활성화 (유전자에서 제거됨)
    ...
    if USE_EXPANDED_DATA:
        ...
        beam_sections_df = pd.read_csv(os.path.join(script_dir, "beam_sections_simple02.csv"))
        column_sections_df = pd.read_csv(os.path.join(script_dir, "column_sections_expanded_rotated.csv"))
        h5_file = h5py.File(os.path.join(script_dir, 'pm_dataset_expanded_rotated.mat'), 'r')
    else:
        print("[Experiment] Using ORIGINAL column dataset.")
        beam_sections_df = pd.read_csv(os.path.join(script_dir, "beam_sections_simple02.csv"))
        column_sections_df = pd.read_csv(os.path.join(script_dir, "column_sections_simple02.csv"))
        h5_file = h5py.File(os.path.join(script_dir, 'pm_dataset_simple02.mat'), 'r')
    ...
    def evaluate(individual, DL, LL, Wx, Wy, Ex, Ey, h5_file, patterns_by_floor):
        ...
        try:
            ...
            beam_indices = individual[len_col_sec:]
            # 시나리오 C에서는 회전 변수가 유전자에서 완전히 제거되었으므로 항상 0으로 설정
            col_rotations = [0] * num_col_groups
        ...
        actual_hierarchy_ratio = max(hierarchy_ratios) if 'hierarchy_ratios' in locals() else 1.0
    ...
    def run_ga_optimization(DL, LL, Wx, Wy, Ex, Ey, crossover_method, patterns_by_floor, h5_file,
        ...
        def custom_mutate(individual, indpb):
            ...
            for i in range(len(individual)):
                # 시나리오 C에서는 회전 변수가 없으므로, 이 부분은 col_sec과 beam_sec에만 적용됩니다.
                # i < chromosome_structure['col_sec'] : 기둥 단면 유전자
                # else : 보 단면 유전자
        ...
        toolbox.register("select_offspring", tools.selTournament, tournsize=3)

        # --- ✅ 4. 최적화 루프 실행 ---
        # 전달받은 초기 개체군을 pop 변수에 할당
        pop = initial_pop
        print(f"\n[Optimization] Starting evolution from generation {start_gen}...")
        ...
        for gen in tqdm(range(start_gen + 1, start_gen + num_generations + 1), desc="세대 진화", unit="gen"):
            ...
            record['hof_size'] = len(hof); record['sep1'] = "|"; record['sep2'] = "|"; record['sep3'] = "|"; record['sep4'] = "|"
    ...
    output_folder = "Results_Scenario_C_02"
    os.makedirs(output_folder, exist_ok=True)
    print(f"\n[Scenario C] Optimization results will be saved in '{output_folder}' folder.")

    # --- DEAP 설정 (run_ga_optimization 호출 전) ---
    creator.create("FitnessMulti", base.Fitness, weights=(-1.0, -1.0))
    creator.create("Individual", list, fitness=creator.FitnessMulti, detailed_results=None)
    ...
    # --- 초기 개체군 생성 및 평가 ---
    print("\n[Optimization] Initializing population...")
    ...
    for ind in tqdm(pop, desc="Evaluating Initial Population", unit="ind"):
        initial_eval_results.append(evaluate(ind, DL_rand, LL_rand, Wx_rand, Wy_rand, Ex_rand, Ey_rand, h5_file, patterns_by_floor))

    for ind, res in zip(pop, initial_eval_results):
        ind.detailed_results = res
    ...
    pop, logbook, hof, hof_stats_history = run_ga_optimization(
        ...
        initial_pop=pop,
        start_gen=0,
        logbook=tools.Logbook(), # 빈 로그북 전달
        hof=hof,
    ...
    log_df.to_csv(os.path.join(output_folder, "optimization_log_C.csv"), index=False)
    if hof_stats_history:
        hof_df = pd.DataFrame(hof_stats_history)
        hof_df.to_csv(os.path.join(output_folder, "hof_convergence_C.csv"), index=False)
    ...
    if valid_solutions:
        ...
        pareto_df.to_csv(os.path.join(output_folder, "pareto_solutions_C.csv"), index=False)
        print(f"Saved {len(pareto_df)} valid Pareto solutions to 'pareto_solutions_C.csv'")

    print("\n" + "="*80)
    print("### Scenario C Optimization Complete ###")
