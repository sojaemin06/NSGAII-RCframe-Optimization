def main():
    ...
    CROSSOVER_STRATEGY = "OnePoint"
    PARENT_SELECTION_CONFIG = {'name': 'Tournament_Size_7', 'func': tools.selTournament, 'params': {'tournsize': 7}}
    population_sizes_to_test = [600, 700, 800]
    NUM_GENERATIONS_FIXED = 100
    ...
    def run_ga_optimization(POPULATION_SIZE, NUM_GENERATIONS, DL, LL, Wx, Wy, Ex, Ey, crossover_method, parent_selection_config, patterns_by_floor, h5_file):
        ...
        for gen in tqdm(range(1, NUM_GENERATIONS + 1), desc=f"Evolving - Pop: {POPULATION_SIZE}"):
    ...
    results_by_pop = {}
    for pop_size in tqdm(population_sizes_to_test, desc="Population Size Experiments"):
        ...
        output_folder = f"paper_figures/population_{pop_size}_02"
        os.makedirs(output_folder, exist_ok=True)
        print(f"\n결과는 '{output_folder}' 폴더에 저장됩니다.")
        print("\n" + "="*90)
        print(f"### 최적화 실행 (Population Size: {pop_size}) ###")
        print(f"- 그룹핑: {GROUPING_STRATEGY}, 교배: {CROSSOVER_STRATEGY}, 부모선택: {PARENT_SELECTION_CONFIG['name']}")
        print(f"- 인구: {pop_size}, 세대: {NUM_GENERATIONS_FIXED}, 교배/변이율: {CXPB}/{MUTPB}")
        ...
        population, logbook, pareto_front, hof_stats_history = run_ga_optimization(
            ...
            NUM_GENERATIONS=NUM_GENERATIONS_FIXED,
        ...
        print("\n" + "="*80)
        print(f"### 최적화 완료 (Population Size: {pop_size}) ###")
        ...
        if not valid_solutions:
            ...
            results_by_pop[f"Pop_{pop_size}"] = {'logbook': logbook, 'pareto_front_results': [], 'hof_stats': hof_stats_history}
        else:
            ...
            results_by_pop[f"Pop_{pop_size}"] = {
                'logbook': logbook,
            ...
            print(f"\n### Population Size {pop_size} 전략 결과 데이터 CSV 저장 ###")
            ...
            print(f"\n### Population Size {pop_size} 전략 결과 시각화 ###")
            
            fig_analysis, axs = plt.subplots(1, 4, figsize=(35, 7))
            fig_analysis.suptitle(f'Optimization Process Analysis (Population Size: {pop_size})', fontsize=20)
        ...
        print(f"\nCOMPLETED RUN for Population Size {pop_size}. Results saved in '{output_folder}'")
    ...
    if results_by_pop:
        ...
        fig.suptitle('Population Size Performance Comparison', fontsize=20)
        
        ax_conv1, ax_conv2, ax_pareto_obj, ax_pareto_real = axes
        
        colors = plt.cm.viridis(np.linspace(0, 1, len(population_sizes_to_test)))
        style_map = {size: {'color': colors[i], 'marker': 'o', 'linestyle': '-'} for i, size in enumerate(population_sizes_to_test)}

        for pop_size_str, results in results_by_pop.items():
            pop_size = int(pop_size_str.split('_')[1])
            style = style_map.get(pop_size)
            ...
            if hof_stats:
                ...
                ax_conv1.plot(gen, best_obj1, label=f"Pop {pop_size}", alpha=0.9, **style)
                ax_conv2.plot(gen, best_obj2, label=f"Pop {pop_size}", alpha=0.9, **style)
        ...
        plt.savefig(os.path.join(output_folder2, "comparison_summary_population_size.png"))
