...
def run_single_experiment(exp_name, pop_size, tourn_size, crossover, cxpb, mutpb, num_gen, common_data):
    ...
    (beam_sections_df, column_sections_df, beam_sections, column_sections,
     ...
     patterns_by_floor_set) = common_data # patterns_by_floor_set 추가
    ...
    _, _, final_hof, hof_stats_history = run_ga_optimization(
        ...
        crossover_method=crossover, patterns_by_floor=patterns_by_floor_set, h5_file=h5_file, # 변환된 set 전달
...
def analyze_and_plot(all_history_data, all_hof_data, step_name, param_key, output_dir):
    ...
    max_hv = -1 # hypervolume은 최소 0 이상이므로 음수 초기화
    
    # final_hvs 딕셔너리의 키가 숫자(예: 0.7)인 경우 max 함수가 인자로 받은 key의 유형을 혼동할 수 있으므로
    # 직접 루프를 돌며 최대값을 찾습니다.
    for label, hv in final_hvs.items():
        if hv > max_hv:
            max_hv = hv
            best_param_label = label

    # best_param이 None일 경우 예외 처리 또는 기본값 설정
    if best_param_label is None:
        print(f"[{step_name} Result] No valid hypervolume found to determine best parameter.")
        return None # 최적 파라미터 찾지 못함
        
    worst_param_label = None
    min_hv = float('inf') # 최저 하이퍼볼륨
    for label, hv in final_hvs.items():
        if hv < min_hv:
            min_hv = hv
            worst_param_label = label

    print(f"\n[{step_name} Result]")
    print(f"  - Best {param_key}: {best_param_label} (HV: {final_hvs[best_param_label]:.4f})")
    print(f"  - Worst {param_key}: {worst_param_label} (HV: {final_hvs[worst_param_label]:.4f})")

    # 3. Pareto Front 비교 그래프 (Best vs Worst)
    plt.figure(figsize=(8, 7))
    
    # Best Param Plot
    best_hof = all_hof_data[best_param_label]
    ...
    plt.scatter(fit2_best, fit1_best, c='blue', label=f'Best ({best_param_label})', alpha=0.7, s=50)

    # Worst Param Plot
    worst_hof = all_hof_data[worst_param_label]
    fit1_worst = [ind.fitness.values[0] for ind in worst_hof]
    fit2_worst = [ind.fitness.values[1] for ind in worst_hof]
    plt.scatter(fit2_worst, fit1_worst, c='red', label=f'Worst ({worst_param_label})', alpha=0.4, marker='x', s=40)
...
def main():
    ...
    # PATTERNS_BY_FLOOR를 set으로 변환
    patterns_by_floor_set = {k: set(v) for k, v in PATTERNS_BY_FLOOR.items()}

    common_data = (beam_sections_df, column_sections_df, beam_sections, column_sections,
                   h5_file, col_map, beam_map, beam_lengths, chromosome_structure,
                   num_columns, num_beams, fixed_min_cost, fixed_range_cost, fixed_min_co2, fixed_range_co2,
                   patterns_by_floor_set) # common_data에 추가
    ...
    try:
        ...
        if history_s2:
            ...
            print("No history data for Step 2. Skipping analysis.")
        ...
        if history_s3:
            ...
            print("No history data for Step 3. Skipping analysis.")
        ...
        if history_s4:
            ...
            print("No history data for Step 4. Skipping analysis.")
        ...
        if history_s5:
            ...
            print("No history data for Step 5. Skipping analysis.")
