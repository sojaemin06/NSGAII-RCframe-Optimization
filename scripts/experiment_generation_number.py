def main():
    ...
    PARENT_SELECTION_CONFIG = {'name': 'Tournament_Size_3', 'func': tools.selTournament, 'params': {'tournsize': 3}}
    generations_to_test = [100, 200, 300, 400, 500]
    POPULATION_SIZE_FIXED = 100
    ...
    h5_file = h5py.File(os.path.join(data_dir, 'pm_dataset_simple02.mat'), 'r')
    ...
    def run_ga_optimization(POPULATION_SIZE, NUM_GENERATIONS, DL, LL, Wx, Wy, Ex, Ey, crossover_method, parent_selection_config, patterns_by_floor, h5_file):
        ...
        for gen in tqdm(range(1, NUM_GENERATIONS + 1), desc=f"Evolving - Gen: {NUM_GENERATIONS}"):
    ...
    results_by_gen = {}
    for num_gen in tqdm(generations_to_test, desc="Generation Number Experiments"):
        ...
        output_folder = f"paper_figures/generation_{num_gen}"
        os.makedirs(output_folder, exist_ok=True)
        print(f"\n결과는 '{output_folder}' 폴더에 저장됩니다.")
        print("\n" + "="*90)
        print(f"### 최적화 실행 (Generation Number: {num_gen}) ###")
        print(f"- 그룹핑: {GROUPING_STRATEGY}, 교배: {CROSSOVER_STRATEGY}, 부모선택: {PARENT_SELECTION_CONFIG['name']}")
        print(f"- 인구: {POPULATION_SIZE_FIXED}, 세대: {num_gen}, 교배/변이율: {CXPB}/{MUTPB}")
        ...
        if not valid_solutions:
            ...
            results_by_gen[f"Gen_{num_gen}"] = {'logbook': logbook, 'pareto_front_results': [], 'hof_stats': hof_stats_history}
        else:
            ...
            print(f"\n### Generation Number {num_gen} 전략 결과 데이터 CSV 저장 ###")
            ...
            print(f"\n### Generation Number {num_gen} 전략 결과 시각화 ###")
            
            fig_analysis, axs = plt.subplots(1, 4, figsize=(35, 7))
            fig_analysis.suptitle(f'Optimization Process Analysis (Generation Number: {num_gen})', fontsize=20)
        ...
        print(f"\nCOMPLETED RUN for Generation Number {num_gen}. Results saved in '{output_folder}'")
    ...
    if results_by_gen:
        ...
        fig.suptitle('Generation Number Performance Comparison', fontsize=20)
        
        ax_conv1, ax_conv2, ax_pareto_obj, ax_pareto_real = axes
        
        colors = plt.cm.viridis(np.linspace(0, 1, len(generations_to_test)))
        style_map = {gen: {'color': colors[i], 'marker': 'o', 'linestyle': '-'} for i, gen in enumerate(generations_to_test)}

        for gen_str, results in results_by_gen.items():
            num_gen = int(gen_str.split('_')[1])
            style = style_map.get(num_gen)
            ...
            if hof_stats:
                ...
                ax_conv1.plot(gen, best_obj1, label=f"Gen {num_gen}", alpha=0.9, **style)
                ax_conv2.plot(gen, best_obj2, label=f"Gen {num_gen}", alpha=0.9, **style)
            ...
            if pareto_results:
                ...
                ax_pareto_obj.scatter(fitness2_vals, fitness1_vals, s=80, edgecolors='k', alpha=0.7, label=f"Gen {num_gen}", color=style['color'], marker=style['marker'])
                costs = [r['cost'] for r in pareto_results]
                co2s = [r['co2'] for r in pareto_results]
                ax_pareto_real.scatter(costs, co2s, s=80, edgecolors='k', alpha=0.7, label=f"Gen {num_gen}", color=style['color'], marker=style['marker'])
        ...
        plt.savefig(os.path.join(output_folder2, "comparison_summary_generation_number.png"))
