def main():
    ...
    # 사용 가능 옵션: "Individual", "Uniform", "ByFloor", "Hybrid"
    crossover_methods_to_test = ["Uniform", "TwoPoint", "OnePoint"]
    ...
    def run_ga_optimization(DL, LL, Wx, Wy, Ex, Ey, crossover_method, patterns_by_floor, h5_file):
        ...
        for gen in tqdm(range(1, NUM_GENERATIONS + 1), desc=f"Evolving - {crossover_method}"):
    ...
    results_by_cx = {}
    for cx_method in tqdm(crossover_methods_to_test, desc="Crossover Strategy Experiments"):
        ...
        output_folder = f"paper_figures/crossover_{cx_method}_02"
        os.makedirs(output_folder, exist_ok=True)
        print(f"\n결과는 '{output_folder}' 폴더에 저장됩니다.")
        print("\n" + "="*90)
        print(f"### 최적화 실행 ({cx_method}) ###")
        print(f"- 그룹핑: {GROUPING_STRATEGY}, 교배: {cx_method}")
        ...
        population, logbook, pareto_front, hof_stats_history = run_ga_optimization(
            DL_rand, LL_rand, Wx_rand, Wy_rand, Ex_rand, Ey_rand, 
        ...
        if not valid_solutions:
            ...
            # Plot 1: Optimization Process
            fig_analysis, axs = plt.subplots(1, 4, figsize=(35, 7))
            fig_analysis.suptitle(f'Optimization Process Analysis ({cx_method})', fontsize=20)
        ...
        print(f"\nCOMPLETED RUN for {cx_method}. Results saved in '{output_folder}'")
    ...
    if results_by_cx:
        ...
        fig.suptitle('Crossover Method Performance Comparison_02', fontsize=20)
        
        ax_conv1, ax_conv2, ax_pareto_obj, ax_pareto_real = axes

        style_map = {
            'Uniform':  {'color': 'C1', 'marker': 's', 'linestyle': '--'},
            'TwoPoint': {'color': 'C2', 'marker': 'P', 'linestyle': ':'},
            'OnePoint': {'color': 'C0', 'marker': 'o', 'linestyle': '-'}
        }

        for method, results in results_by_cx.items():
        ...
        plt.savefig(os.path.join(output_folder2, "comparison_summary_crossover.png"))
